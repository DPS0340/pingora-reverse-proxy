...
impl<A> Service<A> {
    ...
    ///
    /// Unlike [`Self::add_uds`], Pingora performs no pathname permission mutation
    /// when binding or adopting this endpoint.
    #[cfg(unix)]
    pub fn add_uds_with_preconfigured_permissions(&mut self, addr: &str) {
        self.listeners.add_uds_with_preconfigured_permissions(addr);
    }
