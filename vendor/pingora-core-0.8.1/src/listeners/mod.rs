...
impl TransportStackBuilder {
    pub async fn build(
        ...
        #[cfg(unix)]
        builder.uds_permissions_preconfigured(self.uds_permissions_preconfigured);
...
impl Listeners {
    ...
    /// Add a Unix domain socket whose permissions were configured by the caller.
    ///
    /// Pingora will not mutate the socket pathname's permissions while binding or
    /// adopting a pre-bound descriptor. [`Self::add_uds`] retains its existing
    /// default of setting mode 0o666 when `perm` is `None`.
    #[cfg(unix)]
    pub fn add_uds_with_preconfigured_permissions(&mut self, addr: &str) {
        self.stacks.push(TransportStackBuilder {
            l4: ServerAddress::Uds(addr.into(), None),
            tls: None,
            uds_permissions_preconfigured: true,
            #[cfg(feature = "connection_filter")]
