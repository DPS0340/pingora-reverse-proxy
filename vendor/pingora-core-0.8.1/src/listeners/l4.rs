...
mod uds {
    ...
    fn bind_inner(addr: &str, perms: Option<Option<Permissions>>) -> Result<Listener> {
        /*
    ...
    pub(super) fn bind(addr: &str, perms: Option<Permissions>) -> Result<Listener> {
        bind_inner(addr, Some(perms))
    }

    pub(super) fn bind_with_preconfigured_permissions(addr: &str) -> Result<Listener> {
        bind_inner(addr, None)
    }
}
...
fn from_raw_fd(
    ...
    fd: i32,
    uds_permissions_preconfigured: bool,
) -> Result<Listener> {
    match address {
        #[cfg(unix)]
        ServerAddress::Uds(addr, perm) => {
            ...
            if !uds_permissions_preconfigured {
...
async fn bind(addr: &ServerAddress, uds_permissions_preconfigured: bool) -> Result<Listener> {
    match addr {
        #[cfg(unix)]
...
impl ListenerEndpointBuilder {
    pub fn new() -> ListenerEndpointBuilder {
        Self {
            listen_addr: None,
            #[cfg(unix)]
            uds_permissions_preconfigured: false,
            #[cfg(feature = "connection_filter")]
    ...
    #[cfg(unix)]
    pub(crate) fn uds_permissions_preconfigured(&mut self, configured: bool) -> &mut Self {
        self.uds_permissions_preconfigured = configured;
        self
    }
    ...
    pub async fn listen(self, fds: Option<ListenFds>) -> Result<ListenerEndpoint> {
        ...
        let listener = if let Some(fds_table) = fds {
            ...
            if let Some(fd) = table.get(addr_str) {
                from_raw_fd(&listen_addr, *fd, self.uds_permissions_preconfigured)?
            } else {
                // not found
                let listener = bind(&listen_addr, self.uds_permissions_preconfigured).await?;
                table.add(addr_str.to_string(), listener.as_raw_fd());
                listener
            }
        } else {
            // not found, no fd table
            bind(&listen_addr, self.uds_permissions_preconfigured).await?
        };
...
mod test {
    ...
    #[cfg(unix)]
    async fn adopted_uds_mode(permissions_preconfigured: bool) -> u32 {
        use crate::server::Fds;
        use std::os::fd::IntoRawFd;
        use std::os::unix::fs::PermissionsExt;

        let addr = std::env::temp_dir().join(format!(
            "pingora-uds-permissions-{}-{}",
            std::process::id(),
            rand::random::<u64>()
        ));
        let std_listener = StdUnixListener::bind(&addr).expect("bind preconfigured UDS");
        std_listener
            .set_nonblocking(true)
            .expect("set preconfigured UDS nonblocking");
        std::fs::set_permissions(&addr, Permissions::from_mode(0o600))
            .expect("set initial UDS mode");
        let fd = std_listener.into_raw_fd();
        let fds = Arc::new(tokio::sync::Mutex::new(Fds::new()));
        fds.lock()
            .await
            .add(addr.to_string_lossy().into_owned(), fd);

        let mut builder = ListenerEndpoint::builder();
        builder.listen_addr(ServerAddress::Uds(
            addr.to_string_lossy().into_owned(),
            None,
        ));
        builder.uds_permissions_preconfigured(permissions_preconfigured);
        let listener = builder.listen(Some(fds)).await.expect("adopt UDS");
        let mode = std::fs::metadata(&addr)
            .expect("adopted UDS metadata")
            .permissions()
            .mode()
            & 0o777;
        drop(listener);
        std::fs::remove_file(addr).expect("remove adopted UDS");
        mode
    }

    #[cfg(unix)]
    #[tokio::test]
    async fn preconfigured_uds_permissions_skip_pathname_chmod() {
        assert_eq!(adopted_uds_mode(true).await, 0o600);
    }

    #[cfg(unix)]
    #[tokio::test]
    async fn default_none_uds_permissions_remain_0666() {
        assert_eq!(adopted_uds_mode(false).await, 0o666);
    }
