...
CI runs `scripts/verify.sh` on Linux in the required gate order and records exact-SHA verification evidence. Publishing occurs only after a successful CI run for a commit carrying exactly one strict SemVer 2.0 tag. A credential-free trusted rebuild job checks controls out from the immutable `github.workflow_sha`, checks the candidate out separately without persisted credentials, proves that the exact candidate commit belongs to the trusted default-branch history, and uses only the trusted control helper to archive that Git object and build and test its production image. The privileged publication job depends on that job, downloads only its current-run artifact, verifies and loads the tested bytes without rebuilding, and never executes helper code from the candidate checkout. It then pushes the serialized bounded transport reference `candidate-staging`, captures the manifest digest directly from that successful push, verifies the digest-qualified identity, publishes and independently verifies candidate-bound provenance for that digest and exact signer workflow, and only then writes SemVer and SHA discovery tags to the attested digest. A provenance failure therefore leaves no SemVer/SHA release tags; a rerun reuses the mutable staging transport reference but never treats that reference as a trust or deployment identity. “Bounded” means one mutable staging reference, not bounded GHCR blob/manifest storage; repository operators must retain a package policy that expires superseded untagged manifests. Registry errors fail the workflow. GHCR tags are mutable references and may be replaced by a rerun or another authorized registry writer, so they are not a security or immutability boundary. CD never publishes `latest`. Deploy only by the reported `sha256` digest and verify its provenance.

The JupyterHub gate constructs its Docker context only from tracked allowlisted files and rejects tracked symlinks. The authoritative verifier removes ambient `STORE_BACKEND` overrides, requires one complete 18-scenario summary for each of the `memory` and `redis` backends, and rejects a reduced or duplicated matrix. The vendored `pingora-load-balancing 0.8.1` patch is anchored to the checked-in upstream crate archive with SHA-256 `361b69af0234d2e4d10234e2efd106bb3b8147c575d52f45604a46aaf26def7a`; `scripts/verify-vendor-provenance.py` rejects archive drift, unapproved file changes, inventory changes, or drift in any declared patch.
