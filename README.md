...
CI runs `scripts/verify.sh` on Linux in the required gate order. It archives the exact production image exercised by the container gate and records checksums for the archive, saved manifest, image config, image ID, platform, and revision label. Publishing occurs only after a successful CI run for a commit carrying exactly one strict SemVer 2.0 tag. The privileged CD job checks publication helpers out from the immutable `github.workflow_sha`, checks the candidate out separately without persisted credentials, proves that the candidate commit belongs to the trusted default-branch history, and never executes helper code from the candidate checkout. CD then verifies and loads the tested bytes without rebuilding, pushes the serialized bounded transport reference `candidate-staging`, captures the manifest digest directly from that successful push, verifies the digest-qualified identity, publishes and independently verifies provenance for that digest and signer workflow, and only then writes SemVer and SHA discovery tags to the attested digest. A provenance failure therefore leaves no SemVer/SHA release tags; a rerun reuses the mutable staging transport reference but never treats that reference as a trust or deployment identity. “Bounded” means one mutable staging reference, not bounded GHCR blob/manifest storage; repository operators must retain a package policy that expires superseded untagged manifests. Registry errors fail the workflow. GHCR tags are mutable references and may be replaced by a rerun or another authorized registry writer, so they are not a security or immutability boundary. CD never publishes `latest`. Deploy only by the reported `sha256` digest and verify its provenance.
